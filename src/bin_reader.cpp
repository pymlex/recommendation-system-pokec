...
bool load_index_map(const string& idx_path, unordered_map<int, pair<uint32_t,uint32_t> >& out_idx)
{
    ...
    while (getline(in, line)) {
        ...
        string a,b,c;
        if (! (getline(ss, a, ',') && getline(ss, b, ',') && getline(ss, c))) continue;
...
bool read_user_record(const string& bin_path, const unordered_map<int, pair<uint32_t,uint32_t> >& idx_map, int user_id, UserRecord& out_rec)
{
    out_rec = UserRecord();
    if (idx_map.find(user_id) == idx_map.end()) return false;
